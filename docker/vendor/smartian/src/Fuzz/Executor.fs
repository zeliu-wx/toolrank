...
let private resolveAgentPath fileName =
  let srcDir = Directory.GetParent(__SOURCE_DIRECTORY__).FullName
  let fromSource = Path.Join(srcDir, "Agent", fileName)
  if File.Exists(fromSource) then fromSource
  else
    let baseDir = System.AppContext.BaseDirectory
    let candidates = [
      Path.GetFullPath(Path.Combine(baseDir, "..", "src", "Agent", fileName));
      Path.GetFullPath(Path.Combine(baseDir, "..", "..", "src", "Agent", fileName));
      Path.GetFullPath(Path.Combine(baseDir, "src", "Agent", fileName));
      Path.GetFullPath(Path.Combine(baseDir, "Agent", fileName));
    ]
    candidates
    |> List.tryFind File.Exists
    |> Option.defaultValue fromSource

let initialize targetPath =
  targCode <- File.ReadAllText(targetPath) |> hexStrToBytes
  let smartianAgentPath = resolveAgentPath "AttackerContract.bin"
  smartianAgentCode <- File.ReadAllText(smartianAgentPath) |> hexStrToBytes
  let sFuzzAgentPath = resolveAgentPath "SFuzzContract.bin"
