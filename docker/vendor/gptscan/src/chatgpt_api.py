...
DEFAULT_MODEL_GPT4 = "gpt-4"

ENV_API_KEY = "OPENAI_API_KEY"
ENV_API_BASE = "OPENAI_API_BASE"   # common name
ENV_BASE_URL = "OPENAI_BASE_URL"   # alternate name some people use
ENV_MODEL = "GPTSCAN_MODEL"
ENV_MODEL_GPT4 = "GPTSCAN_MODEL_GPT4"
...
class Chat:
    ...
    def sendMessages(self, message:str, GPT4=False) -> str:
        ...
        # Allow using any OpenAI-compatible endpoint (e.g., DashScope/Qwen) via env vars.
        ...
        existing_key = getattr(openai, "api_key", None)
        if GPT4:
            openai.api_key = os.getenv(ENV_API_KEY) or existing_key or GPT4_API
        else:
            fallback_key = OPENAI_APIS[0] if isinstance(OPENAI_APIS, list) and len(OPENAI_APIS) > 0 else None
            openai.api_key = os.getenv(ENV_API_KEY) or existing_key or fallback_key
