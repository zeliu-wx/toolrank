...
def _resolve_timeout(default_timeout: float) -> float:
    raw = os.environ.get("GPTSCAN_TIMEOUT_SECONDS", "").strip()
    if not raw:
...
def _resolve_use_gpt4(default_flag: bool) -> bool:
    raw = os.environ.get("GPTSCAN_USE_GPT4", "").strip().lower()
    if not raw:
...
def _pick_best_json_obj(candidates: List[dict], expected_keys: List[str]) -> dict:
    if not candidates:
        raise ValueError("No JSON object found in model response")
    if expected_keys:
        expected = {k for k in expected_keys if isinstance(k, str)}
...
def ask_for_static_json(prompts, source, answer_keys):
    ...
    data = _pick_best_json_obj(candidates, answer_keys)
...
def ask_for_static_json_single(prompts, source, answer_key):
    ...
    if answer_key in data:
        return {answer_key: data[answer_key]}
    if len(data) == 0:
