...
def _fuzzy_get(mapping: object, key: object):
...
def _validate_with_description(
    raw: object,
    answer: Dict,
    validate_cfg: Dict,
) -> Tuple[bool, str]:
    if not isinstance(validate_cfg, dict):
        return True, ""
    for to_validate_key, to_validate_values in validate_cfg.items():
        selected = answer.get(to_validate_key)
        raw_group = _fuzzy_get(raw, to_validate_key)
        desc = ""
        if isinstance(raw_group, dict):
            desc_obj = _fuzzy_get(raw_group, selected)
            if desc_obj is not None:
                desc = str(desc_obj).lower()
        if not desc:
            return False, f"missing description for key={to_validate_key}, selected={selected}"
        for v_line in to_validate_values:
            if not isinstance(v_line, list):
                v_line = [v_line]
            hit = False
            for v in v_line:
                if str(v).lower() in desc:
                    hit = True
                    break
            if not hit:
                return False, f"description check failed for key={to_validate_key}, selected={selected}"
    return True, ""


def _validate_with_exclude(
    answer: Dict,
    exclude_cfg: Dict,
) -> Tuple[bool, str]:
    if not isinstance(exclude_cfg, dict):
        return True, ""
    for to_exclude_key, to_exclude_values in exclude_cfg.items():
        val = str(answer.get(to_exclude_key, "")).lower()
        for banned in to_exclude_values:
            if str(banned).lower() in val:
                return False, f"excluded value matched for key={to_exclude_key}, value={answer.get(to_exclude_key)}"
    return True, ""
                                        ...
                                        if "multisteps" in vul["static"] and vul["static"]["multisteps"] == True:
                                            ...
                                            if "filter" in vul["static"]:
                                                for filter_variable in vul["static"]["filter"]:
                                                    if filter_variable in answer:
                                                        ...
                                                        logger.warning(f"Filter variable not found: {filter_variable}")
                                            ...
                                            if "format" in vul["static"] and vul["static"]["format"] == "json":
                                                ...
                                                if "validate_description" in vul["static"]:
                                                    ok, reason = _validate_with_description(
                                                        raw,
                                                        answer,
                                                        vul["static"]["validate_description"],
                                                    )
                                                    if not ok:
                                                        logger.warning(
                                                            f"validate_description failed; file={file}, function={function1}, vul={vul['name']}, reason={reason}"
                                                        )
                                                        skip_static_check = True
                                                if not skip_static_check and "exclude_variable" in vul["static"]:
                                                    ok, reason = _validate_with_exclude(
                                                        answer,
                                                        vul["static"]["exclude_variable"],
                                                    )
                                                    if not ok:
                                                        logger.warning(
                                                            f"exclude_variable failed; file={file}, function={function1}, vul={vul['name']}, reason={reason}"
                                                        )
                                                        skip_static_check = True
                                        ...
                                        if "multisteps" in vul["static"] and vul["static"]["multisteps"] == True:
                                            ...
                                            if "format" in vul["static"] and vul["static"]["format"] == "json":
                                                ...
                                                if "validate_description" in vul["static"]:
                                                    ...
                                                    if not ok:
                                                        logger.warning(
                                                            f"validate_description failed; file={function2_file}, function={function2_func}, vul={vul['name']}, reason={reason}"
                                                        )
                                                        skip_static_check = True
                                                if not skip_static_check and "exclude_variable" in vul["static"]:
                                                    ok, reason = _validate_with_exclude(
                                                    ...
                                                    if not ok:
                                                        logger.warning(
                                                            f"exclude_variable failed; file={function2_file}, function={function2_func}, vul={vul['name']}, reason={reason}"
                                                        )
                                                        skip_static_check = True
                                            elif "format" in vul["static"] and vul["static"]["format"] == "json_single":
